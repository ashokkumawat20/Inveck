...
public class ExShortProcessOnLine extends AppCompatActivity implements SwipeRefreshLayout.OnRefreshListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_ex_short_process_on_line);
        ...
        if (AppStatus.getInstance(ExShortProcessOnLine.this).isOnline()) {
            new getSummaryStockList().execute();
            new getStockList().execute();
        } else {

            Toast.makeText(ExShortProcessOnLine.this, Constant.INTERNET_MSG, Toast.LENGTH_SHORT).show();
        }
        Continue.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                AlertDialog.Builder builder = new AlertDialog.Builder(ExShortProcessOnLine.this);
                builder.setMessage("Do you want to take sign Off ?")
                        .setCancelable(false)
                        .setPositiveButton("Continue", new DialogInterface.OnClickListener() {
                            public void onClick(DialogInterface dialog, int id) {
                                dialog.cancel();
                                Intent intent = new Intent(ExShortProcessOnLine.this, TakeSignOffActivity.class);
        ...
        filterData.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                final PopupMenu popup = new PopupMenu(ExShortProcessOnLine.this, filterData);
                ...
                popup.setOnMenuItemClickListener(new PopupMenu.OnMenuItemClickListener() {
                    public boolean onMenuItemClick(MenuItem item) {
                        ...
                        if (item.getTitle().equals("All")) {
                            adapter = new TotalStockRecyclerAdapter(ExShortProcessOnLine.this, data);
                            mList.setLayoutManager(new LinearLayoutManager(ExShortProcessOnLine.this));
                            ...
                            adapter = new TotalStockRecyclerAdapter(ExShortProcessOnLine.this, filteredList);
                            mList.setLayoutManager(new LinearLayoutManager(ExShortProcessOnLine.this));
                            mList.setAdapter(adapter);
                            adapter.notifyDataSetChanged();  // data set changed
        ...
        sortData.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                final PopupMenu popup = new PopupMenu(ExShortProcessOnLine.this, sortData);
                //Inflating the Popup using xml file
        ...
        LinearLayoutManager layoutManager = new LinearLayoutManager(ExShortProcessOnLine.this);
        mList.setLayoutManager(layoutManager);

        adapter = new TotalStockRecyclerAdapter(ExShortProcessOnLine.this, new ArrayList<>());
        mList.setAdapter(adapter);
        ...
        protected Void doInBackground(Void... params) {
            ...
            if (stockListResponse.compareTo("") != 0) {
                ...
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        Toast.makeText(ExShortProcessOnLine.this, "Please check your network connection.", Toast.LENGTH_LONG).show();
                    }
                });
    ...
    private class getStockList extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... params) {
            WebClient serviceAccess = new WebClient();
            Log.i("json", "json" + jsonLeadObj);
            stockListResponse = serviceAccess.SendHttpGet(Config.URL_MILESTONEDETAILS + "/EXSSORT/" + Integer.parseInt(preferences.getString("inventry_user_id", "")) + "/" + preferences.getInt("milestone_Id", 0) + "/" + preferences.getInt("client_Id", 0) + "/" + currentPage, jsonLeadObj);
            Log.i("resp", stockListResponse);
            if (stockListResponse.compareTo("") != 0) {
                if (isJSONValid(stockListResponse)) {
